...
void OnStartCombat(const std::string& enemyId) {
    if (enemyId == "mortal_thug") {
        auto cs = std::make_unique<CombatState>(L"七玄门恶徒", 60, 10, 15, 5, EnemyType::Mortal);
        GameStateManager::Instance().PushState(std::move(cs));
    } else if (enemyId == "street_rogue") {
        auto cs = std::make_unique<CombatState>(L"街头流氓", 45, 5, 12, 3, EnemyType::Mortal);
        GameStateManager::Instance().PushState(std::move(cs));
    } else if (enemyId == "qi_disciple") {
        auto cs = std::make_unique<CombatState>(L"黄枫谷弟子", 120, 40, 28, 15, EnemyType::Qi);
        GameStateManager::Instance().PushState(std::move(cs));
    } else if (enemyId == "spirit_beast") {
        auto cs = std::make_unique<CombatState>(L"低阶灵兽", 90, 20, 22, 12, EnemyType::YaoBeast);
        GameStateManager::Instance().PushState(std::move(cs));
    } else if (enemyId == "blood_spider") {
        auto cs = std::make_unique<CombatState>(L"血色蜘蛛", 100, 25, 26, 10, EnemyType::YaoBeast);
        GameStateManager::Instance().PushState(std::move(cs));
